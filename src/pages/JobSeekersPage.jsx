...
import { Bookmark, Briefcase, MapPin, Search, Filter, UploadCloud, ArrowRight, Building } from 'lucide-react';
...
const All_JOB_TYPE_VALUE = "all-job-type-placeholder";
...
const JobSeekersPage = () => {
  ...
  const [filters, setFilters] = useState({ role: '', location: ALL_LOCATIONS_VALUE, jobType: All_JOB_TYPE_VALUE });
  ...
  const uniqueLocations = useMemo(() => {
    ...
    jobs.forEach(job => {
      ...
      if (
        loc &&                            // not null/undefined
        loc.trim() !== '' &&              // not empty string
        loc.trim().toLowerCase() !== 'n/a' // not 'N/A'
      ) {
  ...
  // const uniqueJobType = useMemo(() => [...new Set(jobs.map(job => job.job_type))], [jobs]);
  const uniqueJobType = useMemo(() => {
    const normalized = jobs
      .map(job => job.job_type)
      .filter(type => type && type.trim().toLowerCase() !== 'n/a')
      .map(type => type.trim().toLowerCase());

    const unique = [...new Set(normalized)];

    // Optionally capitalize for display
  ...
  useEffect(() => {
    ...
    if (filters.location && filters.location !== ALL_LOCATIONS_VALUE) {
      ...
      console.log("Filtered by location:", filters.location, currentJobs);
    }
    if (filters.jobType && filters.jobType !== All_JOB_TYPE_VALUE) {
      console.log("Current Jobs before filter:", currentJobs);
      currentJobs = currentJobs.filter(job =>
        job.job_type &&
        job.job_type.trim().toUpperCase() === filters.jobType.trim().toUpperCase()
      );
    }
    ...
    console.log('Filters applied:', filters, 'Search term:', searchTerm, 'Result:', currentJobs);
  }, [filters, searchTerm, jobs]);

  const handleFilterChange = (filterName, value) => {
    console.log('Filter change:', filterName, value);
    console.log('Previous filters:', filters);
    console.log('Unique sets:', uniqueLocations, uniqueJobType);
          ...
          <div className="grid md:grid-cols-4 gap-4 items-end">
            ...
            <div>
              ...
              <Select value={filters.jobType} onValueChange={(value) => handleFilterChange('jobType', value)}>
                <SelectTrigger id="jobType"><SelectValue placeholder="All" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value={All_JOB_TYPE_VALUE}>All</SelectItem>
                  {uniqueJobType.map(jobType => <SelectItem key={jobType} value={jobType}>{jobType}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-4 gap-8 mb-5">
            {filteredJobs.map((job, index) => (
              <JobListCard
                ...
                jobId={job.id}
                icon={Bookmark}
                jobTitle={job.title ? job.title : 'N/A'}
                ...
                description={job.description ? job.description.substring(0, 60) + '...' : ''}
                jobType={job.job_type ? job.job_type : 'N/A'}
                jobLevel={job.job_level ? job.job_level : 'N/A'}
                jobMode={job.job_mode ? job.job_mode : 'N/A'}
